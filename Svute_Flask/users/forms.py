...
class Update_Account_Form(FlaskForm):
    ...
    picture = FileField('Cập nhật ảnh đại diện', validators=[FileAllowed(['jpg', 'png', 'svg'])])
    ...
    current_pass = PasswordField('Mật khẩu', validators=[Length(min=6, max=50, message='Mật khẩu có độ dài từ 6 đến 50 ký tự!')])
    current_pass = PasswordField('Mật khẩu', validators=[Length(min=6, max=50, message='Mật khẩu có độ dài từ 6 đến 50 ký tự!')])
    def validate_username(self, username):
